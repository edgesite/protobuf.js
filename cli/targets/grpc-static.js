...
function buildService(ref, service) {
    push(`class ${escapeName(service.name)} {`);
    indent++;
    pushComment([
        ...
        "@constructor",
        "@param {grpc.Client} client gRPC client",
    ]);
    push(`constructor(client) { this.$client = client; }`)

    service.methodsArray.forEach(function(method) {
        method.resolve();
        const serviceName = exportName(service);
        const lcName = protobuf.util.lcFirst(method.name);
        const reqName = exportName(method.resolvedRequestType, !config.forceMessage);
        const respName = exportName(method.resolvedResponseType, !config.forceMessage);
        push("");
        pushComment([
            ...
            '@instance',
            `@param {${reqName}} request ${method.resolvedRequestType.name} message or plain object`,
            '@param {grpc.Metadata?} metadata',
            '@param {{deadline?: grpc.Deadline, host?: string, parent?: grpc.Call, propagate_flags?: number, credentials?: grpc.CallCredentials}?} options',
            `@returns {Promise<${respName}>} Promise`,
        ]);
        push(`${escapeName(lcName)}(request, metadata, options) {`);
        indent++;
        push('return new Promise((resolve, reject) => {')
        indent++;
        push(`this.$client.makeUnaryRequest('/${serviceName}/${method.name}', ${reqName}, ${respName}, request, metadata, options, (err, resp) => err ? reject(err) : resolve(resp));`);
        indent--;
        push('});')
        indent--;
        push(`}`);
